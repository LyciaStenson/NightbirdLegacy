...
void main() {
	...
	// Set up ray origin and direction for a simple ray marching effect
	...
	// Limit depth for color attenuation
	depth = min(6.0, depth);
	vec3 n = calcNormal(p);
	float b = max(0.0, dot(n, vec3(0.577)));
	vec3 col = (0.5 + 0.5 * cos((b + uTime * 3.0) + vUv.xyx * 2.0 + vec3(0, 2, 4))) * (0.85 + b * 0.35);
	col *= exp(-depth * 0.15);

	// Use alpha channel to represent thickness
